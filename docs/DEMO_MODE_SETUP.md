...
> See also the original incident writeup `docs/ECS_DEMO_MODE_SETUP_AND_TROUBLESHOOTING.md`. **This document supersedes its env-loading guidance:** the current build *does* load `.env` automatically via `app/env_bootstrap.py`, so you no longer have to `export` flags in the same shell (though that still works).
...
```
If `/dashboard` returns `401`, the flags did not reach the process — see `docs/TROUBLESHOOTING_GUIDE.md` §1–§3.
...
# ... see docs/ENVIRONMENT_CONFIGURATION.md §5
```

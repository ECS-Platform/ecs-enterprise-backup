...
**Grounding:** `config/integrations.yaml` (`prisma_cloud`), `ecs_platform/ingestion.py`. Part of [Integrations Index](_legacy_INTEGRATIONS_index.md). Interface-complete, `enabled: false` by default.

## Architecture
ECS connector → Prisma Cloud API (`${PRISMA_URL}`) → pulls CSPM findings → evidence → repository. Primary source for **Cloud Security** posture (see [Cloud Security framework](../../../01-product/product/CLOUD_SECURITY.md)).

...
|---|---|---|
| **Two RBAC systems** | canonical `config/rbac.yaml` (`rbac_catalog`) **+** legacy `app/role_permissions.py` **and** `modules/shared/services/role_permissions.py` | Migrate fully to YAML catalog; retire both legacy modules (see `docs/developer-manual/ECS_RBAC_LEGACY_FLAWS.md`) |
...
| Duplicate EA review | root `architecture/…` + `docs/architecture/…` | Retire root stray |

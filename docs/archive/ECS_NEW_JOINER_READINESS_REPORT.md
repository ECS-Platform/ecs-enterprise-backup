...
|---|:--:|---|---|
| **Install ECS** | ✅ Yes | `docs/developer-manual/DEVELOPER_SETUP_GUIDE.md` (macOS/Linux/WSL, exact commands), `requirements.txt` (9 deps), `docker-compose.yml` | None material |
| **Run ECS** | ✅ Yes | `README.md` quick start, `docs/00-start-here/DEMO_MODE_SETUP.md`, `docs/00-start-here/COMMON_COMMANDS.md`; health checks `/healthz` `/readyz` | None material |
| **Understand ECS** | ✅ Yes | `docs/00-start-here/ARCHITECTURE_OVERVIEW.md`, `docs/hld`, `docs/lld`, `docs/product_manual/*`, diagrams | Two systems (legacy RBAC + canonical YAML) need the handbook to reconcile |
| **Debug ECS** | ✅ Yes | `docs/00-start-here/TROUBLESHOOTING_GUIDE.md`, `docs/operations/ecs_runbook.md`, 39 test files | No central log-reading guide; covered partly in runbook |
| **Demo ECS** | ✅ Yes | `docs/product/ECS_DEMO_GUIDE.md`, `demo-data/ECS_DEMO_NARRATIVE.md`, validator → READY | None material |
| **Extend ECS** | ⚠️ Mostly | `docs/developer-manual/ECS_ENGINEERING_HANDBOOK.md`, `ECS_MODULE_OWNERSHIP.md`, module structure is consistent | No formal "add a module/connector/framework" tutorial; no release process doc |

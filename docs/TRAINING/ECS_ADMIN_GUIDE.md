...
- **Legacy system:** `role_permissions.py` still present; normalized for compatibility (see `docs/ECS_RBAC_LEGACY_FLAWS.md`). **Do not add new rules to the legacy path.**
...
See `docs/RECOVERY_RUNBOOK.md` — Postgres evidence repository backup/restore/migration (authoritative).
...
See `docs/COMMON_COMMANDS.md` (start/stop/test/seed/backup/reset). Tests: `./venv/bin/pytest` (39 suites).

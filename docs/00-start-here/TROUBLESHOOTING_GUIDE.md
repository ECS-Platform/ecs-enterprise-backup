...
Each entry is **Symptom → Root cause → Resolution → Verification**, derived from the actual code paths (`app/env_bootstrap.py`, `app/main.py`, `routes_platform.py`, `docker-compose.yml`, the seed scripts, and `docs/00-start-here/ECS_DEMO_MODE_SETUP_AND_TROUBLESHOOTING.md`).
...
Always read the **startup banner** first — `ecs_lifespan` prints the effective `DEMO_MODE`, `ECS_AUTH_ENABLED`, and how `.env` was parsed. Most "it won't load" issues are an env-not-applied problem (entries 1–3 above). For the full demo-mode playbook see `docs/00-start-here/DEMO_MODE_SETUP.md`.

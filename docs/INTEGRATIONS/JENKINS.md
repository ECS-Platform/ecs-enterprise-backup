...
**Grounding:** `config/integrations.yaml` (`jenkins`), `ecs_platform/ingestion.py`, `docker-compose.yml` (`jenkins` service, `demo-connectors` profile). Part of [Integrations Index](README.md). Self-hostable — **`enabled: true` by default for development**.

...
with a new authoritative [`docs/03-development/developer-manual/ECS_ENGINEERING_HANDBOOK.md`](../../03-development/developer-manual/ECS_ENGINEERING_HANDBOOK.md) that
...
| `docs/03-development/developer-manual/DEVELOPER_SETUP_GUIDE.md` | Setup (macOS/Linux/WSL) | **Current, authoritative** | Best setup doc; matches code. |
| `docs/03-development/developer-manual/LOCAL_DEVELOPMENT_GUIDE.md` | Dev workflow | **Current, authoritative** | Best workflow doc; matches code. |
| `docs/01-product/00-start-here/ECS_DEMO_MODE_SETUP_AND_TROUBLESHOOTING.md` | Demo mode + troubleshooting | **STALE / partially incorrect** | Claims `.env` is not loaded — false (see §5, §7). |
| `docs/02-architecture/architecture/ECS_Architecture_and_Deployment_Guide.md` | Architecture + deploy | **Partially stale** | States 4 deps (actual 9), "no Docker", "no .env" — predates Docker stack & dotenv. |
| `docs/03-development/developer-manual/ECS_DEPENDENCY_REPORT.md` | Cross-module coupling | Current (analysis) | Refactor-oriented; not onboarding. |
| `docs/03-development/developer-manual/ECS_MODULE_OWNERSHIP.md` | Ownership + branch/PR rules | Current (proposal) | Only place with branching/PR guidance (recommended, not enforced). |
| `docs/03-development/operations/RECOVERY_RUNBOOK.md` | Backup/restore/migration | **Current, authoritative** | Accurate, scoped to Postgres repository. |
| `docs/05-archive/archive/ECS_MIGRATION_REPORT.md` | Refactor history | Current (historical) | Background only. |
| `docs/03-development/developer-manual/ECS_REFACTOR_PLAN.md` | Refactor plan | Current (historical) | Background only. |
| `docs/03-development/developer-manual/ECS_RBAC_LEGACY_FLAWS.md` | RBAC analysis | Current (historical) | Background only. |
| `docs/05-archive/archive/ECS_ROLLBACK_REPORT.md` | Rollback history | Current (historical) | Background only. |
| `docs/02-architecture/architecture/ecs_enterprise_architecture_review.md` | EA review | Current | Architecture package #1. |
| `docs/02-architecture/architecture/ecs_deployment_architecture.md` | Deployment arch | Current | Architecture package #6. |
| `docs/02-architecture/architecture/ecs_hld.md` | HLD | Current | Architecture package #2. |
| `docs/02-architecture/architecture/ecs_lld.md` | LLD | Current | Architecture package #3. |
| `docs/02-architecture/diagrams/ecs_er_diagrams.md` | ER diagrams | Current | Architecture package #4. |
| `docs/02-architecture/diagrams/ecs_sequence_diagrams.md` | Sequences | Current | Architecture package #5. |
| `docs/05-archive/archive/ecs_technology_dossier.md` | Exec dossier | Current | Architecture package #7. |
| `docs/03-development/operations/ecs_runbook.md` | Ops runbook | Current | Architecture package #8 (operational troubleshooting). |
| `executive/*` (7 files) | Exec/board/ROI/pitch | Current | Non-engineering; `documentation_audit.md` is a prior consistency audit. |
| `architecture/ecs_enterprise_architecture_review.md` (root) | EA review | **Duplicate** | Same title as `docs/02-architecture/architecture/...`; root copy is the stray. |
...
| Topic | Where | Verdict |
|---|---|---|
| New-engineer setup (macOS/Linux/Windows-WSL) | `docs/03-development/developer-manual/DEVELOPER_SETUP_GUIDE.md` | **Complete & accurate.** Per-OS prerequisites, venv, deps, `.env`, health check, post-setup checklist. |
...
| As-built setup (legacy) | `docs/02-architecture/architecture/ECS_Architecture_and_Deployment_Guide.md` §13 | **Stale** — says 4 deps, "no Docker", uses `.venv`/`wkin_ecs_consolidated_demo_v13`. |
...
| Topic | Where | Verdict |
|---|---|---|
| Env var table (full) | **Missing** — `README.md` links to `docs/03-development/developer-manual/ENVIRONMENT_CONFIGURATION.md` which **does not exist**. | **Gap.** |
...
| Backup/repository vars | `docs/03-development/operations/RECOVERY_RUNBOOK.md` §0 | Complete for repository vars. |
...
| Topic | Where | Verdict |
|---|---|---|
| Demo-mode purpose & flags | `docs/01-product/00-start-here/ECS_DEMO_MODE_SETUP_AND_TROUBLESHOOTING.md` | **Stale** — see §5/§7. Correct on *which* flags; wrong on *how env is loaded*. |
...
| Canonical demo-mode doc | `README.md` links to `docs/01-product/00-start-here/DEMO_MODE_SETUP.md` which **does not exist** | **Gap.** |
...
| Caveat | Detail |
|---|---|
| `docs/02-architecture/architecture/ECS_Architecture_and_Deployment_Guide.md` | Useful as-built/target comparison, but stale on deps/Docker/env. |
| `README.md` links `docs/01-product/00-start-here/ARCHITECTURE_OVERVIEW.md` | **Does not exist** — the short engineer-facing overview is missing (the full package exists, but no one-page entry). |
| Root `architecture/...` | Duplicate of `docs/02-architecture/architecture/...`. |

## 5. Existing troubleshooting documentation

| Topic | Where | Verdict |
|---|---|---|
| Demo-mode 401 / env not visible | `docs/01-product/00-start-here/ECS_DEMO_MODE_SETUP_AND_TROUBLESHOOTING.md` | **Partially obsolete.** Symptom is real; stated root cause ("no dotenv") is wrong for the current build. |
| As-built symptom table | `docs/02-architecture/architecture/ECS_Architecture_and_Deployment_Guide.md` §19 | Useful, generic. |
| Operational troubleshooting | `docs/03-development/operations/ecs_runbook.md` | Current. |
| Setup-failure pointer | `DEVELOPER_SETUP_GUIDE.md` → links `docs/01-product/00-start-here/TROUBLESHOOTING_GUIDE.md` which **does not exist** | **Gap.** |
...
2. **`docs/03-development/developer-manual/ENVIRONMENT_CONFIGURATION.md`** — linked by README, **not created**. No full env-var reference.
3. **`docs/01-product/00-start-here/DEMO_MODE_SETUP.md`** — linked by README, **not created**.
4. **`docs/01-product/00-start-here/COMMON_COMMANDS.md`** — linked by README, **not created**. No command catalog (start/stop/test/seed/backup).
5. **`docs/01-product/00-start-here/TROUBLESHOOTING_GUIDE.md`** — linked by README & setup guide, **not created**.
6. **`docs/01-product/00-start-here/ARCHITECTURE_OVERVIEW.md`** — linked by README, **not created** (the deep package exists, the one-pager doesn't).
...
| D5 | **EA review duplicated** | root `architecture/...` vs `docs/02-architecture/architecture/...` | Keep `docs/` copy; retire root stray. |
...
1. **Create one authoritative source:** [`docs/03-development/developer-manual/ECS_ENGINEERING_HANDBOOK.md`](../../03-development/developer-manual/ECS_ENGINEERING_HANDBOOK.md)

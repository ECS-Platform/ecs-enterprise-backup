...
**Type:** Master KPI reference. **No code/UI/DB changes.** **Grounding:** dashboard engines (`demo_metrics`, `executive_analytics_engine`, `trends_analytics_engine`, `evidence_health_engine`, `evidence_approval_engine`, `governance_completeness_engine`, `framework_kpi_drill_engine`, `ecs_ai_governance_drilldowns`, `ecs_platform/governance.py`). Builds on [audit KPI dictionary](../archive/AUDIT_ECS_KPI_DICTIONARY.md) and [product-manual KPI dictionary](../product/PRODUCT_MANUAL_ECS_KPI_DICTIONARY.md) — this version adds **Refresh Frequency · Business Impact · Executive Usage · Associated Dashboard/Workflow/Framework** per KPI.
...
## Cross-references
- Audit-grade gap analysis: [ECS_KPI_DICTIONARY.md (AUDIT)](../archive/AUDIT_ECS_KPI_DICTIONARY.md)

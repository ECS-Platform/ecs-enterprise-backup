...
- **Legacy system:** `role_permissions.py` still present; normalized for compatibility (see `docs/03-development/developer-manual/ECS_RBAC_LEGACY_FLAWS.md`). **Do not add new rules to the legacy path.**
...
See `docs/03-development/operations/RECOVERY_RUNBOOK.md` — Postgres evidence repository backup/restore/migration (authoritative).
...
See `docs/01-product/00-start-here/COMMON_COMMANDS.md` (start/stop/test/seed/backup/reset). Tests: `./venv/bin/pytest` (39 suites).

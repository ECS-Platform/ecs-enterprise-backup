...
**66 screenshots** of the running platform (demo mode) are captured under `docs/01-product/product/screenshots/`. The full index with descriptions and section references is in **`ECS_SCREENSHOTS_INDEX.md`**. A few anchors:
...
A structured plan to go from zero to self-sufficient. Each day: read, then *do* in demo mode (`DEMO_MODE=true`, see `docs/01-product/00-start-here/DEMO_MODE_SETUP.md`).

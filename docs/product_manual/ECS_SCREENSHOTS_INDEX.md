...
- **Server:** ECS run locally with `DEMO_MODE=true ECS_AUTH_ENABLED=false uvicorn app.main:app` on `127.0.0.1:8000` (so every persona can reach every screen — see `docs/DEMO_MODE_SETUP.md`).
- **Location:** `docs/product_manual/screenshots/`
...
The repository also contains 7 earlier screenshots under `demo-data/screenshots/` (e.g. `01-executive-summary.png`, `11-evidence-reuse.png`, `12-scorecard-cio.png`, `14-framework-coverage.png`, `15-ai-assistant-pci.png`). Those are retained for the demo narrative; the authoritative product-manual set is the 66 images above under `docs/product_manual/screenshots/`.

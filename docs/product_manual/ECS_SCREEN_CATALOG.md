...
**Conventions.** Most screens accept `role` and `user` query params (and often `notice`/`response`). Browser routes return server-rendered HTML; `/api/*` return JSON (see `ECS_FEATURE_REFERENCE.md` for the API/drill catalog). Screenshot file names refer to `docs/product_manual/screenshots/` (see `ECS_SCREENSHOTS_INDEX.md`).

...
**Type:** Use case catalog (150+). **No code/UI/DB changes.** **Grounding:** ECS screens/modules/workflows/personas (see [Master Product Manual](ECS_MASTER_PRODUCT_MANUAL.md), [User Journeys](../product_manual/ECS_USER_JOURNEYS.md)). AI-specific catalog (100+) is in [docs/AI Catalog V2](../AI/ECS_LOCAL_LLM_USE_CASE_CATALOG_V2.md) — this master catalog spans **all** domains and cross-references it. ROI grounded in `config/roi.yaml` model (hours saved × rate; reuse multiplier); values **[Inferred/Target — validate]**.
...
## J. AI (UC-AI01..AI11) — see [AI Catalog V2](../AI/ECS_LOCAL_LLM_USE_CASE_CATALOG_V2.md) for 100+
...
- **UC-W01** Application onboarding workflow · Ops · app → governed app · Onboarding · coverage. (see [guide](../operations/ECS_APPLICATION_ONBOARDING_GUIDE.md))
...
## ROI model note
Per `config/roi.yaml` (`roi/workbook.py`): value = hours-saved × rate + reuse savings; surfaced on [ROI Center](ECS_MASTER_PRODUCT_MANUAL.md). **A documented rate-basis discrepancy exists** (`cost_per_hour: 1500` vs ₹1,000/hr tables) — see [documentation audit](../../executive/documentation_audit.md); quote with that caveat. Per-use-case ROI figures are **[Inferred/Target — validate]**.

## Cross-references
- Screens: [ECS_MASTER_PRODUCT_MANUAL.md](ECS_MASTER_PRODUCT_MANUAL.md) · KPIs: [ECS_MASTER_KPI_DICTIONARY.md](ECS_MASTER_KPI_DICTIONARY.md) · AI use cases: [Catalog V2](../AI/ECS_LOCAL_LLM_USE_CASE_CATALOG_V2.md) · Journeys: [User Journeys](../product_manual/ECS_USER_JOURNEYS.md)

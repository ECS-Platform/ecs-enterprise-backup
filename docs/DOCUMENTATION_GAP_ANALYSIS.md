...
with a new authoritative [`docs/ECS_ENGINEERING_HANDBOOK.md`](ECS_ENGINEERING_HANDBOOK.md) that
...
| `docs/DEVELOPER_SETUP_GUIDE.md` | Setup (macOS/Linux/WSL) | **Current, authoritative** | Best setup doc; matches code. |
| `docs/LOCAL_DEVELOPMENT_GUIDE.md` | Dev workflow | **Current, authoritative** | Best workflow doc; matches code. |
| `docs/ECS_DEMO_MODE_SETUP_AND_TROUBLESHOOTING.md` | Demo mode + troubleshooting | **STALE / partially incorrect** | Claims `.env` is not loaded — false (see §5, §7). |
| `docs/ECS_Architecture_and_Deployment_Guide.md` | Architecture + deploy | **Partially stale** | States 4 deps (actual 9), "no Docker", "no .env" — predates Docker stack & dotenv. |
| `docs/ECS_DEPENDENCY_REPORT.md` | Cross-module coupling | Current (analysis) | Refactor-oriented; not onboarding. |
| `docs/ECS_MODULE_OWNERSHIP.md` | Ownership + branch/PR rules | Current (proposal) | Only place with branching/PR guidance (recommended, not enforced). |
| `docs/RECOVERY_RUNBOOK.md` | Backup/restore/migration | **Current, authoritative** | Accurate, scoped to Postgres repository. |
| `docs/ECS_MIGRATION_REPORT.md` | Refactor history | Current (historical) | Background only. |
| `docs/ECS_REFACTOR_PLAN.md` | Refactor plan | Current (historical) | Background only. |
| `docs/ECS_RBAC_LEGACY_FLAWS.md` | RBAC analysis | Current (historical) | Background only. |
| `docs/ECS_ROLLBACK_REPORT.md` | Rollback history | Current (historical) | Background only. |
...
| `docs/hld/ecs_hld.md` | HLD | Current | Architecture package #2. |
| `docs/lld/ecs_lld.md` | LLD | Current | Architecture package #3. |
...
| `docs/executive/ecs_technology_dossier.md` | Exec dossier | Current | Architecture package #7. |
...
| Topic | Where | Verdict |
|---|---|---|
| New-engineer setup (macOS/Linux/Windows-WSL) | `docs/DEVELOPER_SETUP_GUIDE.md` | **Complete & accurate.** Per-OS prerequisites, venv, deps, `.env`, health check, post-setup checklist. |
...
| As-built setup (legacy) | `docs/ECS_Architecture_and_Deployment_Guide.md` §13 | **Stale** — says 4 deps, "no Docker", uses `.venv`/`wkin_ecs_consolidated_demo_v13`. |
...
| Topic | Where | Verdict |
|---|---|---|
| Env var table (full) | **Missing** — `README.md` links to `docs/ENVIRONMENT_CONFIGURATION.md` which **does not exist**. | **Gap.** |
...
| Backup/repository vars | `docs/RECOVERY_RUNBOOK.md` §0 | Complete for repository vars. |
...
| Topic | Where | Verdict |
|---|---|---|
| Demo-mode purpose & flags | `docs/ECS_DEMO_MODE_SETUP_AND_TROUBLESHOOTING.md` | **Stale** — see §5/§7. Correct on *which* flags; wrong on *how env is loaded*. |
...
| Canonical demo-mode doc | `README.md` links to `docs/DEMO_MODE_SETUP.md` which **does not exist** | **Gap.** |
...
| Caveat | Detail |
|---|---|
| `docs/ECS_Architecture_and_Deployment_Guide.md` | Useful as-built/target comparison, but stale on deps/Docker/env. |
| `README.md` links `docs/ARCHITECTURE_OVERVIEW.md` | **Does not exist** — the short engineer-facing overview is missing (the full package exists, but no one-page entry). |
...
| Topic | Where | Verdict |
|---|---|---|
| Demo-mode 401 / env not visible | `docs/ECS_DEMO_MODE_SETUP_AND_TROUBLESHOOTING.md` | **Partially obsolete.** Symptom is real; stated root cause ("no dotenv") is wrong for the current build. |
| As-built symptom table | `docs/ECS_Architecture_and_Deployment_Guide.md` §19 | Useful, generic. |
| Operational troubleshooting | `docs/operations/ecs_runbook.md` | Current. |
| Setup-failure pointer | `DEVELOPER_SETUP_GUIDE.md` → links `docs/TROUBLESHOOTING_GUIDE.md` which **does not exist** | **Gap.** |
...
2. **`docs/ENVIRONMENT_CONFIGURATION.md`** — linked by README, **not created**. No full env-var reference.
3. **`docs/DEMO_MODE_SETUP.md`** — linked by README, **not created**.
4. **`docs/COMMON_COMMANDS.md`** — linked by README, **not created**. No command catalog (start/stop/test/seed/backup).
5. **`docs/TROUBLESHOOTING_GUIDE.md`** — linked by README & setup guide, **not created**.
6. **`docs/ARCHITECTURE_OVERVIEW.md`** — linked by README, **not created** (the deep package exists, the one-pager doesn't).
...
1. **Create one authoritative source:** [`docs/ECS_ENGINEERING_HANDBOOK.md`](ECS_ENGINEERING_HANDBOOK.md)

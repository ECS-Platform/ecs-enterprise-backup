...
**Mode:** READ-ONLY / ANALYSIS / REPORTING. **No code/RBAC changes. No commits.** **Grounding:** `config/rbac.yaml` (legacy `rbac.roles` = 9 roles; canonical `rbac_catalog.roles`; `rbac_catalog.pages` = 7 dashboards; `rbac_legacy_compat.aliases`), `nav_audit/persona_shots/*`, `nav_audit/persona_validation_report.md`, `nav_audit/persona_drill_matrix.md`. Complements [Persona Guide](../product_manual/ECS_PERSONA_GUIDE.md).
...
## Cross-references
- [Persona Guide](../product_manual/ECS_PERSONA_GUIDE.md) · [Role Action Matrix](../WORKFLOWS/ECS_ROLE_ACTION_MATRIX.md) · [Security Reference](../SECURITY/ECS_SECURITY_REFERENCE.md) · `nav_audit/persona_validation_report.md`

...
**Mode:** READ-ONLY / ANALYSIS / REPORTING. **No connector implementation changes. No commits.** **Grounding:** `ecs_platform/connectors/*` (real connector clients), `ecs_platform/ingestion.py` (`sync_connector`), `ecs_platform/connectors/http_client.py` + `_msgraph.py`, `modules/operations/engines/query_connectors.py` (interfaces only), `config/integrations.yaml`, `config/environments/_base.yaml`. Complements [Integrations Index](../INTEGRATIONS/README.md).
...
## Cross-references
- [Integrations Index](../INTEGRATIONS/README.md) · [Predefined Query Readiness](ECS_PREDEFINED_QUERY_READINESS_REPORT.md) · [Environment Framework Review](ECS_ENVIRONMENT_FRAMEWORK_REVIEW.md) · [Connector Failure Playbook](ECS_CONNECTOR_FAILURE_PLAYBOOK.md)

...
def enterprise_widgets_context(role: str = "", page_module: str = "", framework: str = "", user: str = ""):
    ...
    ctx = {
        ...
        "governance_analytics": build_governance_analytics(),
    }
    if page_module:
        ctx["nav_module"] = page_module
        ctx["module_view"] = get_module_capability(page_module, role)
        ctx["show_mvp_notifications"] = True
    if framework:
        from app.workflow_module import build_universal_workflow_rows

        ctx["page_workflow_items"] = build_universal_workflow_rows(
            framework=framework, role=role, limit=30
        )
        ctx["page_workflow_title"] = f"Pending workflow items — {framework}"
        ctx["show_framework_workflow"] = True

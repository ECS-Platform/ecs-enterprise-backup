...
from modules.governance.engines.analytics_module import application_comparison, enterprise_dashboard
from modules.executive_overview.engines.demo_metrics import BUSINESS_UNITS, display_framework_maturity, enterprise_kpis
...
def update_chat_context(user: str, role: str, query: str = "", **extra) -> dict:
    ...
    from modules.shared.services.chatbot_context_engine import parse_query_context
...
def _itpp_answer(q: str, role: str) -> str:
    ...
    if "validation" in q and "fail" in q:
        from modules.frameworks.engines.control_validation_engine import failed_validations
...
def _validation_answer(q: str, role: str, framework: str) -> str | None:
    if "validation" not in q and "failed validation" not in q and "controls failed" not in q:
        return None
    from modules.frameworks.engines.control_validation_engine import failed_validations, validation_summary
    fw = framework or ""
...
def _grc_platform_answer(q: str, role: str) -> str | None:
    from modules.enterprise_grc.engines.correlation_engine import find_correlations_by_tool
    from modules.enterprise_grc.engines.enterprise_grc import build_cmdb_inventory, build_exceptions_td, build_executive_heatmaps, build_risk_register
...
def process_query(query: str, role: str = "owner", user: str = "User") -> str:
    ...
    from modules.shared.services.chatbot_context_engine import render_governance_panel

...
from modules.executive_overview.engines.demo_metrics import REUSE_METRICS, enterprise_kpis
from modules.operations.engines.evidence_repository import where_else_used
...
from modules.shared.services.chatbot_engine import CLARIFICATION_PROMPT
...
def try_enhanced_answer(query: str, role: str = "owner", user: str = "User") -> str | None:
    from modules.operations.engines.operations_intelligence import try_operations_answer

    ops_answer = try_operations_answer(query, role=role, user=user)
    if ops_answer:
        return ops_answer

    from modules.shared.services.chatbot_engine import process_query
    ...
    if "trend" in q or "closure" in q:
        from modules.governance.engines.analytics_module import compliance_trends
    ...
    if "notification" in q or "activity" in q:
        from modules.shared.services.audit_trail import get_recent_activity
    ...
    if "aging" in q or "sla" in q:
        from modules.executive_overview.engines.demo_metrics import AUDIT_AGING_BUCKETS
    ...
    if any(w in q for w in ("stale evidence", "failed evidence", "high-risk evidence", "high risk evidence")) or (
        ...
        from modules.shared.services.chatbot_nav import evidence_health_link, mvp_url, link_html

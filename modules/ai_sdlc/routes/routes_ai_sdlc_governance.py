...
from modules.ai_sdlc.engines.ai_sdlc_governance_service import (
    ...
    controlled_document_view,
    control_drill_view,
    evidence_view,
    evidence_viewer_view,
    findings_view,
    landing_view,
    observation_drill_view,
    onboarding_view,
    onboarding_run_view,
...
def register_ai_sdlc_routes(app, templates):
    ...
    @app.get("/api/ai-sdlc/controlled-document")
    def api_controlled_document(
        stage: str = "",
        application: str = "",
        framework: str = "",
        domain: str = "",
        ...
        if not stage or not application or not framework or not control_id:
            return JSONResponse({"ok": False, "error": "stage, application, framework, and control_id required"})
        doc = controlled_document_view(stage, application, framework, domain, control_id, control_name)
        if not doc.get("ok"):
            return JSONResponse({"ok": False, "error": doc.get("error", "Document not found")})
        return JSONResponse({"ok": True, "data": doc})

    @app.get("/api/ai-sdlc/control-drill")
    def api_control_drill(
        application: str = "",
        framework: str = "",
        domain: str = "",
        control_id: str = "",
        ...
        if not application or not framework or not control_id:
            return JSONResponse({"ok": False, "error": "application, framework, and control_id required"})
        return JSONResponse({"ok": True, "data": control_drill_view(application, framework, domain, control_id, control_name)})

    @app.get("/api/ai-sdlc/observation-drill")
    def api_observation_drill(
        observation_id: str = "",
        application: str = "",
        framework: str = "",
        control_id: str = "",
        ...
        if not observation_id:
            return JSONResponse({"ok": False, "error": "observation_id required"})
        return JSONResponse({"ok": True, "data": observation_drill_view(observation_id, application, framework, control_id)})

    @app.get("/api/ai-sdlc/controlled-document/counts")
    def api_controlled_document_counts():
        return JSONResponse({"ok": True, "data": controlled_document_counts()})

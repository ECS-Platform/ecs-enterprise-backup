...
def export_onboarding_summary(result: dict[str, Any]) -> str:
    ...
    for g in result["remediation_gaps"]:
        lines.append(f"• {g}")
    lines.extend(_control_evaluation_summary_lines(result.get("control_evaluation")))
    lines.extend(["", "Generated by ECS Onboarding Simulator"])
    return "\n".join(lines)


def _control_evaluation_summary_lines(ce: dict[str, Any] | None) -> list[str]:
    """Common-Control evaluation section for the onboarding export package.

    Additive to the existing plain-text export — only appears when a
    ``control_evaluation`` report (real, PQ-evidence-driven; see
    ``modules.operations.engines.common_control_onboarding.onboard_from_intake_payload``)
    is attached to the onboarding result. Deliberately never claims a
    framework is "compliant" from partial control coverage — same honesty
    rule the evaluation engine itself enforces.
    """
    if not ce or not ce.get("ok"):
        return []
    if ce.get("note"):
        return ["", "REUSABLE COMMON CONTROL EVALUATION", "-" * 42, ce["note"]]

    lines = [
        "",
        "REUSABLE COMMON CONTROL EVALUATION",
        "-" * 42,
        f"Assets evaluated: {', '.join(a['asset_id'] + ' (' + a['technology'] + ')' for a in ce.get('assets', []))}",
        f"Coverage: {ce.get('coverage_pct', 0)}% of applicable reusable controls implemented",
    ]
    for bucket, label in (
        ("implemented", "Implemented"),
        ("partial", "Partial"),
        ("not_implemented", "Not Implemented"),
        ("unknown", "Unknown / Connectivity Pending"),
        ("not_applicable", "Not Applicable"),
    ):
        rows = ce.get(bucket) or []
        if rows:
            lines.append(f"{label}: " + ", ".join(r["control"] for r in rows))
    if ce.get("frameworks"):
        lines.extend(["", "Per-Framework Coverage (reusable controls only — not full framework compliance):"])
        for fw in ce["frameworks"]:
            lines.append(
                f"  {fw['framework_name']}: {fw['implemented']}/{fw['controls_mapped']} controls implemented "
                f"({fw['coverage_pct']}%) — {fw['partial']} partial, {fw['unknown']} unknown"
            )
    if ce.get("connectivity_pending"):
        lines.append("")
        lines.append(f"Connectivity pending on {len(ce['connectivity_pending'])} control/technology combination(s) — re-run once targets are reachable.")
    return lines


def recent_onboarding_suggestions(onboarded: list[str]) -> list[str]:
    """Autocomplete suggestions for the Application Name field."""
    return ["NetBanking", "Mobile Banking", "Payments"]


#: Autocomplete suggestions for the Database Technology field — restricted to
#: technologies with real EAR/EIT (and audit-logging/password-policy/
#: privileged-access) evaluators in config/common_control_rules.yaml, so
#: whatever the user picks actually maps to an evaluable Common Control
#: rather than falling through as NOT_APPLICABLE.
DATABASE_TECHNOLOGY_SUGGESTIONS: list[str] = ["Aurora MySQL", "YugabyteDB", "PostgreSQL", "MySQL"]

#: Autocomplete suggestions for the OS / Linux Technology field — restricted
#: to the technologies config/common_control_rules.yaml actually has
#: CC-SECURE_CONFIGURATION and/or CC-VULNERABILITY_PATCH rules for (Linux,
#: Red Hat Enterprise Linux 8.x, Red Hat Enterprise Linux 9.x). Short forms
#: ("RHEL 8" / "RHEL 9") are used deliberately — the full canonical label
#: "Red Hat Enterprise Linux 8.x" typed verbatim does not round-trip through
#: technology_fingerprint.fingerprint_asset's text-match rules (it matches
#: the generic "linux" pattern before the "rhel.?8" one), while "RHEL 8"
#: canonicalizes correctly to "Red Hat Enterprise Linux 8.x". Note RHEL 9.x
#: has no CC-VULNERABILITY_PATCH rule in the pack (Linux/RHEL 8 only) — that
#: control legitimately stays NOT_APPLICABLE for RHEL 9, not a gap here.
OS_TECHNOLOGY_SUGGESTIONS: list[str] = ["Linux", "RHEL 8", "RHEL 9"]
